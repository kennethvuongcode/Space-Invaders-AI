...
class Linear_QNet(nn.Module):
    ...
    #     x = self.output(x)  # Output layer without non-linear activation
    #     return x
    ...
    def save(self,file_name = 'model.pth'):  #TODO: update file name
        model_folder_path = './model'
        if not os.path.exists(model_folder_path):
            os.makedirs(model_folder_path)
            
        file_name = os.path.join(model_folder_path, file_name)
        torch.save(self.state_dict(), file_name)        #Updating model parameters
...
class QTrainer:
    ...
    def train_step(self,state,action,reward,next_state,done):
        ...
        #if multiple values already in size (n,x)
        ...
        if len(state.shape) == 1:
            ...
            # 1: predicted Q values with current stat                                                                                                                                                                                                                                                                                                                                                                     
